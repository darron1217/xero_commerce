...
var Category = (function (XE, $, Tree) {
  ...
  return {
    ...
    _$wrap.on('change', '.imageUpload', function (e) {
		...
		$parent.find('.delLogo').prop('disabled', true);
        $parent.find('.imagePreview').attr('src', URL.createObjectURL(e.target.files[0]));
        $parent.find('.imagePreview').css('display', 'inline');
    });

    //사진 삭제 버튼 클릭 했을 때 처리
    _$wrap.on('click', '.imageDelete', function () {
		var $parent = $(this).closest('.form-group');
		$parent.find('.imageUpload').val('');
        $parent.find('.delLogo').prop('disabled', false);
        $parent.find('.imagePreview').attr('src', '');
        $parent.find('.imagePreview').css('display', 'none');
    });
	...
	<div class="form-group">
		<label>이미지</label>
		<small>(300px * 300px)</small>
		<br>
		<img class="imagePreview" style="display:inline; max-width: 300px; max-height: 300px;" name="image" src="">
		<input class="delImage" type="hidden" name="image" value="del" disabled>
		<input class="imageUpload" type="file" name="image" ><br>
		<button type="button" class="btn xe-danger imageDelete">삭제</button>
	</div>
`
      template += '</div>'
      template += '</form>'
